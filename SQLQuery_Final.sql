...
--add quarters
SELECT [date], year_name,
DATENAME(quarter, date) AS [quarter_num]
FROM [Orders];


ALTER TABLE Orders
ADD quarter_num NVARCHAR(50);


UPDATE Orders
SET quarter_num = DATENAME(quarter, date);

--total revenue by quarters
SELECT year_name, quarter_num, SUM(transaction_amount) AS quarter_revenue
from orders
Group by quarter_num, year_name;
